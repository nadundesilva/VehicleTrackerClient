...
.controller('miscellaneousCostsCtrl', function($scope, $rootScope, $stateParams, $http, $state, $ionicHistory, $cordovaToast, $timeout, ionicMaterialMotion, ionicMaterialInk) {
  ...
  $scope.loadFuelFillUps = function() {
    $scope.showLoadingOverlay('Retrieving Miscellaneous Costs');
    $http.get($rootScope.MAIN_SERVER_URL + '/vehicle/' + $scope.selected_vehicle.license_plate_no + '/misc-cost/')
      .then(function (response) {
        if (response.data.status == 'SUCCESS') {
          $scope.misc_costs = response.data.misc_costs;
          $scope.triggerAnimation();
        } else if (response.data.status == 'USER_NOT_LOGGED_IN') {
          $ionicHistory.nextViewOptions({
            disableBack: true
